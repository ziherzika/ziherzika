...
import {
  ...
  LogOut, Music2, Pause, Play, Plus, Search, Send, X,
} from "lucide-react";
...
function App() {
  ...
  async function saveTrack(form:HTMLFormElement){
    if(!activeProjectId){setNewTrack(false);flash("Prvo otvori projekt.");setView("projects");return}
  ...
  return <div className="stage"><main className="app-shell">
    {view==="home"&&<><header className="home-topbar"><div><p className="eyebrow">{activeProject?.name||"ZIHERZIKA"}</p><h1>Tvoje trake</h1></div><button className="avatar" onClick={()=>setProfileOpen(true)}>B</button></header><button className="searchbox" onClick={()=>setView("search")}><Search size={18}/>Pretraži trake</button>{tracks.length?<TrackList title={workspaceLoading?"Učitavam…":`${tracks.length} ${tracks.length===1?"traka":"trake"}`} tracks={tracks} onSelect={chooseTrack}/>:<section className="empty-workspace"><Music2/><h1>{workspaceLoading?"Učitavam trake…":activeProject?"Ovaj projekt još nema traka.":"Prvo napravi projekt."}</h1>{!workspaceLoading&&<><p>{activeProject?"Otvori projekt i tamo dodaj prvu traku.":"Trake uvijek žive unutar projekta."}</p><button className="primary" onClick={()=>activeProject?void openProject(activeProject):setNewProject(true)}>{activeProject?"Otvori projekt":"Novi projekt"}</button></>}</section>}</>}
    {view==="track"&&selected&&<>
    ...
    {view==="project"&&<>
      <PageHeader title={activeProject?.name||"Projekt"} onBack={()=>setView("projects")}/>
      ...
      <div className="tabs"><button className={projectTab==="tracks"?"active":""} onClick={()=>setProjectTab("tracks")}>Trake <b>{tracks.length}</b></button><button className={projectTab==="chat"?"active":""} onClick={()=>setProjectTab("chat")}>Chat <b>{messages.length}</b></button></div>
    ...
    {view==="project"&&projectTab==="tracks"&&activeProjectId&&<button className="fab" onClick={()=>setNewTrack(true)} aria-label="Dodaj traku"><Plus/></button>}
