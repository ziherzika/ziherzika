...
import {
  ArrowLeft, BarChart3, ChevronRight, FileText, Folder, Home, MessageCircle, MoreHorizontal,
  LogOut, Music2, Pause, Play, Plus, Search, Send, Trash2, UserRound, X,
} from "lucide-react";
...
type ApiChatMessage = { id:string; userId:string; author:string; body:string; createdAt:number };
type ProfileStats = { words:number; tracks:number; personalTracks:number; projectTracks:number; projects:number; ownedProjects:number; messages:number; averageWords:number };
...
function messageFromApi(message:ApiChatMessage,mine=false):ChatMessage{return{id:message.id,author:message.author,initials:message.author.split(/\s+/).map(part=>part[0]).join("").slice(0,2).toUpperCase()||"?",text:message.body,time:new Date(message.createdAt).toLocaleTimeString("hr-HR",{hour:"2-digit",minute:"2-digit"}),mine}}
...
function App() {
  ...
  const [view,setView]=useState<"home"|"track"|"projects"|"project"|"search"|"profile">("home");
  ...
  const [profile,setProfile]=useState({id:"",displayName:"",email:""});
  const [stats,setStats]=useState<ProfileStats>({words:0,tracks:0,personalTracks:0,projectTracks:0,projects:0,ownedProjects:0,messages:0,averageWords:0});
  ...
  async function loadProjectMessages(projectId:string,token:string){const response=await fetch(`${API_URL}/api/projects/${projectId}/chat`,{headers:{Authorization:`Bearer ${token}`}});if(!response.ok)throw new Error("Chat se ne može učitati.");const data=await response.json() as {messages?:ApiChatMessage[]};setMessages((data.messages||[]).map(item=>messageFromApi(item,item.userId===profile.id)))}
  async function loadProfileStats(token:string){const response=await fetch(`${API_URL}/api/stats`,{headers:{Authorization:`Bearer ${token}`}});if(!response.ok)throw new Error("Statistika se ne može učitati.");const data=await response.json() as {profile:{id:string;displayName:string;email:string};stats:ProfileStats};setProfile(data.profile);setStats(data.stats)}
  async function loadWorkspace(token:string){setWorkspaceLoading(true);try{const [projectsResponse]=await Promise.all([fetch(`${API_URL}/api/projects`,{headers:{Authorization:`Bearer ${token}`}}),loadPersonalTracks(token),loadProfileStats(token)]);if(projectsResponse.status===401){sessionStorage.removeItem("ziherzika_token");sessionStorage.removeItem("ziherzika_project");setSignedIn(false);return}if(!projectsResponse.ok)throw new Error("Projekti se ne mogu učitati.");const projectsData=await projectsResponse.json() as {projects?:ApiProject[]};const realProjects=(projectsData.projects||[]).map((project,index)=>({id:project.id,name:project.name,subtitle:`${project.members} ${project.members===1?"član":"člana"}`,members:project.members,unread:0,color:index%2?"blue":"orchid"}));setProjects(realProjects);const remembered=sessionStorage.getItem("ziherzika_project"),projectId=realProjects.some(project=>project.id===remembered)?remembered:realProjects[0]?.id;if(!projectId){sessionStorage.removeItem("ziherzika_project");setActiveProjectId(null);setProjectTracks([]);return}sessionStorage.setItem("ziherzika_project",projectId);setActiveProjectId(projectId)}catch(error){flash(error instanceof Error?error.message:"Podaci se ne mogu učitati.")}finally{setWorkspaceLoading(false)}}
  ...
  async function openProfile(){setView("profile");const token=sessionStorage.getItem("ziherzika_token");if(token)try{await loadProfileStats(token)}catch(error){flash(error instanceof Error?error.message:"Statistika se ne može učitati.")}}
  async function openProject(project:Project){setActiveProjectId(project.id);sessionStorage.setItem("ziherzika_project",project.id);setView("project");setProjectTab("tracks");const token=sessionStorage.getItem("ziherzika_token");if(!token)return;setWorkspaceLoading(true);try{await Promise.all([loadProjectTracks(project.id,token),loadProjectMessages(project.id,token)])}catch(error){flash(error instanceof Error?error.message:"Projekt se ne može učitati.")}finally{setWorkspaceLoading(false)}}
  ...
  async function addMessage(){const body=message.trim(),token=sessionStorage.getItem("ziherzika_token");if(!body||!token||!activeProjectId)return;setMessage("");try{const response=await fetch(`${API_URL}/api/projects/${activeProjectId}/chat`,{method:"POST",headers:{"content-type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({body})});const data=await response.json() as {message?:ApiChatMessage;error?:string};if(!response.ok||!data.message)throw new Error(data.error||"Poruka nije poslana.");setMessages(items=>[...items,{...messageFromApi(data.message!),mine:true}])}catch(error){setMessage(body);flash(error instanceof Error?error.message:"Poruka nije poslana.")}}
  ...
  return <div className="stage"><main className="app-shell">
    ...
    {view==="profile"&&<><PageHeader title="Profil" onBack={()=>setView("home")}/><section className="profile-page"><div className="profile-identity"><div className="big-avatar">{profile.displayName.slice(0,1).toUpperCase()||"Z"}</div><h2>{profile.displayName||"Tvoj profil"}</h2><p>{profile.email}</p></div><div className="words-hero"><span><FileText/></span><div><small>UKUPNO NAPISANO</small><strong>{stats.words.toLocaleString("hr-HR")}</strong><p>riječi u tekstovima</p></div></div><div className="stats-grid"><StatCard icon={<Music2/>} value={stats.tracks} label="napravljenih traka"/><StatCard icon={<Folder/>} value={stats.projects} label={`projekata · ${stats.ownedProjects} tvojih`}/><StatCard icon={<MessageCircle/>} value={stats.messages} label="poruka u chatu"/><StatCard icon={<Home/>} value={stats.personalTracks} label="osobnih traka"/><StatCard icon={<BarChart3/>} value={stats.projectTracks} label="projektnih traka"/><StatCard icon={<FileText/>} value={stats.averageWords} label="riječi po traci"/></div><button className="profile-logout" onClick={()=>{sessionStorage.removeItem("ziherzika_token");sessionStorage.removeItem("ziherzika_project");setSignedIn(false)}}><LogOut/> Odjavi se <ChevronRight/></button></section></>}
...
function StatCard({icon,value,label}:{icon:React.ReactNode;value:number;label:string}){return <article className="stat-card"><span>{icon}</span><strong>{value.toLocaleString("hr-HR")}</strong><small>{label}</small></article>}
