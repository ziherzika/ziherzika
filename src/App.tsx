...
function App() {
  ...
  if(!signedIn)return <AuthScreen onDone={async(token)=>{sessionStorage.setItem("ziherzika_token",token);await loadWorkspace(token);setSignedIn(true)}} onDemo={()=>{setProjects(demoProjects);setActiveProjectId(demoProjects[0].id);setTracks(initialTracks);setSelected(null);setSignedIn(true)}}/>;
