...
export class ChatRoom extends DurableObject<Env> {
  ...
  countMessagesForUser(userId:string): number {
    return this.ctx.storage.sql.exec<{count:number}>("SELECT COUNT(*) count FROM messages WHERE user_id=?",userId).one().count;
  }

  async deleteRoom(): Promise<void> {
    for (const socket of this.ctx.getWebSockets()) socket.close(1001,"Projekt je obrisan");
...
async function route(request:Request,env:Env):Promise<Response>{
  ...
  if(path==="/api/stats"&&request.method==="GET") return getStats(user,env);
  if(path==="/api/invites"&&request.method==="GET") return listInvites(user,env);
  const inviteActionMatch=path.match(/^\/api\/invites\/([^/]+)\/(accept|decline)$/);
  if(inviteActionMatch&&request.method==="POST") return respondToInvite(inviteActionMatch[1],inviteActionMatch[2] as "accept"|"decline",user,env);
  ...
  if(personalTrackMatch&&request.method==="DELETE") return deletePersonalTrack(personalTrackMatch[1],user,env);
  ...
  if(projectMatch&&request.method==="DELETE") return deleteProject(projectMatch[1],user,env);
  const projectInvitesMatch=path.match(/^\/api\/projects\/([^/]+)\/invites$/);
  if(projectInvitesMatch&&request.method==="POST") return createInvite(request,projectInvitesMatch[1],user,env);
  const projectMembersMatch=path.match(/^\/api\/projects\/([^/]+)\/members$/);
  if(projectMembersMatch&&request.method==="GET") return listMembers(projectMembersMatch[1],user,env);
  ...
  if(trackMatch&&request.method==="DELETE") return deleteProjectTrack(trackMatch[1],trackMatch[2],user,env);
...
async function listInvites(user:User,env:Env){const rows=await env.DB.prepare("SELECT i.id,i.project_id,p.name project_name,u.display_name invited_by,i.role,i.created_at FROM project_email_invites i JOIN projects p ON p.id=i.project_id JOIN users u ON u.id=i.created_by WHERE i.invited_user_id=? AND i.status='pending' ORDER BY i.created_at DESC").bind(user.id).all();return json({invites:rows.results})}
async function createInvite(request:Request,projectId:string,user:User,env:Env){const role=await membership(projectId,user.id,env);if(role!=="owner")throw new HttpError(403,"Samo vlasnik može pozvati članove.");const body=await readJson<{email?:string}>(request),email=(body.email||"").trim().toLowerCase();if(!/^\S+@\S+\.\S+$/.test(email))throw new HttpError(400,"Unesi ispravan email.");const invited=await env.DB.prepare("SELECT id FROM users WHERE email=?").bind(email).first<{id:string}>();if(!invited)throw new HttpError(404,"Korisnik s tim emailom još nije registriran.");if(invited.id===user.id)throw new HttpError(400,"Već si član ovog projekta.");const existingMember=await env.DB.prepare("SELECT 1 present FROM project_members WHERE project_id=? AND user_id=?").bind(projectId,invited.id).first();if(existingMember)throw new HttpError(409,"Korisnik je već član projekta.");const id=crypto.randomUUID(),now=Date.now();await env.DB.prepare("INSERT INTO project_email_invites(id,project_id,invited_user_id,created_by,role,status,created_at,responded_at) VALUES(?,?,?,?,?,'pending',?,NULL) ON CONFLICT(project_id,invited_user_id) DO UPDATE SET created_by=excluded.created_by,role=excluded.role,status='pending',created_at=excluded.created_at,responded_at=NULL").bind(id,projectId,invited.id,user.id,"editor",now).run();return json({invite:{id,email,role:"editor"}},201)}
async function respondToInvite(inviteId:string,action:"accept"|"decline",user:User,env:Env){const invite=await env.DB.prepare("SELECT id,project_id,role FROM project_email_invites WHERE id=? AND invited_user_id=? AND status='pending'").bind(inviteId,user.id).first<{id:string;project_id:string;role:string}>();if(!invite)throw new HttpError(404,"Poziv više nije aktivan.");const now=Date.now();if(action==="accept")await env.DB.batch([env.DB.prepare("INSERT OR IGNORE INTO project_members(project_id,user_id,role,joined_at) VALUES(?,?,?,?)").bind(invite.project_id,user.id,invite.role,now),env.DB.prepare("UPDATE project_email_invites SET status='accepted',responded_at=? WHERE id=?").bind(now,invite.id)]);else await env.DB.prepare("UPDATE project_email_invites SET status='declined',responded_at=? WHERE id=?").bind(now,invite.id).run();return json({invite:{id:invite.id,status:action==="accept"?"accepted":"declined"}})}
async function getStats(user:User,env:Env){const [personalNotes,projectNotes,memberships,owned]=await Promise.all([env.DB.prepare("SELECT notes FROM personal_tracks WHERE owner_id=?").bind(user.id).all<{notes:string}>(),env.DB.prepare("SELECT notes FROM tracks WHERE created_by=?").bind(user.id).all<{notes:string}>(),env.DB.prepare("SELECT project_id FROM project_members WHERE user_id=?").bind(user.id).all<{project_id:string}>(),env.DB.prepare("SELECT COUNT(*) count FROM projects WHERE owner_id=?").bind(user.id).first<{count:number}>()]);const projectIds=memberships.results.map(row=>row.project_id),messageCounts=await Promise.all(projectIds.map(id=>env.CHAT_ROOMS.getByName(id).countMessagesForUser(user.id))),personalTracks=personalNotes.results.length,projectTracks=projectNotes.results.length,tracks=personalTracks+projectTracks,words=[...personalNotes.results,...projectNotes.results].reduce((sum,row)=>sum+countWords(row.notes),0);return json({profile:{id:user.id,displayName:user.display_name,email:user.email},stats:{words,tracks,personalTracks,projectTracks,projects:projectIds.length,ownedProjects:owned?.count||0,messages:messageCounts.reduce((sum,count)=>sum+count,0),averageWords:tracks?Math.round(words/tracks):0}})}
...
async function deletePersonalTrack(trackId:string,user:User,env:Env){const result=await env.DB.prepare("DELETE FROM personal_tracks WHERE id=? AND owner_id=?").bind(trackId,user.id).run();if(!result.meta.changes)throw new HttpError(404,"Traka ne postoji.");return json({deleted:true})}
...
async function listMembers(id:string,user:User,env:Env){await membership(id,user.id,env);const rows=await env.DB.prepare("SELECT u.id,u.display_name,u.email,pm.role FROM project_members pm JOIN users u ON u.id=pm.user_id WHERE pm.project_id=? ORDER BY CASE pm.role WHEN 'owner' THEN 0 WHEN 'editor' THEN 1 ELSE 2 END,u.display_name").bind(id).all();return json({members:rows.results})}
async function deleteProject(id:string,user:User,env:Env){const role=await membership(id,user.id,env);if(role!=="owner")throw new HttpError(403,"Samo vlasnik može obrisati projekt.");await env.CHAT_ROOMS.getByName(id).deleteRoom();const result=await env.DB.prepare("DELETE FROM projects WHERE id=? AND owner_id=?").bind(id,user.id).run();if(!result.meta.changes)throw new HttpError(404,"Projekt ne postoji.");return json({deleted:true})}
...
async function deleteProjectTrack(projectId:string,trackId:string,user:User,env:Env){const role=await membership(projectId,user.id,env);if(role==="viewer")throw new HttpError(403,"Nemaš dopuštenje za brisanje trake.");const result=await env.DB.prepare("DELETE FROM tracks WHERE id=? AND project_id=?").bind(trackId,projectId).run();if(!result.meta.changes)throw new HttpError(404,"Traka ne postoji.");return json({deleted:true})}
...
function countWords(value:string){const clean=value.trim();return clean?clean.split(/\s+/u).length:0}
